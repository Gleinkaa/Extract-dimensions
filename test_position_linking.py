...
Requires PyMuPDF (fitz) — silently skipped if not installed.
"""
...
def _drawing(path: Path) -> None:
    ...
    page.draw_circle(fitz.Point(40, 100), 10, color=(0, 0, 0), width=1)   # r=10
    page.draw_rect(fitz.Rect(100, 80, 150, 110), color=(0, 0, 0), width=1)  # 50x30
    # Text callouts near their measured feature.
    page.insert_text(fitz.Point(32, 106), "Ø20", fontsize=8)   # diameter 20 → circle
    page.insert_text(fitz.Point(38, 96), "R10", fontsize=8)    # radius 10 → circle
    page.insert_text(fitz.Point(118, 74), "50 mm", fontsize=8)  # generic 50 → rect
    doc.save(str(path))
    doc.close()


def test_position_aware_parse_and_link() -> None:
    assert fitz is not None, "PyMuPDF required to run this test"
